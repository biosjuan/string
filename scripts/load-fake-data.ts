...
async function loadFakeData(numUsers: number = 10) {
  ...
  try {
    ...
    const res = await client.query(
      "select id from public.users order by create_at desc limit $1",
      [numUsers]
    );
...
const numbers = parseInt(process.argv[2]) || 10;
console.log(`loading ${numbers} fake users`);

loadFakeData(numbers);
