...
export async function GET(
  ...
  const querySelect = `select * from posts where id = $1 and user_id = $2`;
  const res = await sql(querySelect, [params.id, jwtPayload.sub]);
  if (res.rowCount === 0) {
...
export async function PATCH(
  ...
  return NextResponse.json({ msg: "udpate success" }, { status: 201 });
}
